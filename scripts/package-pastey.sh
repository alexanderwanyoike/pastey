...
CREATE_UPDATER_ARTIFACTS="${PASTEY_CREATE_UPDATER_ARTIFACTS:-0}"
BUNDLE_KIND="${PASTEY_BUNDLE_KIND:-}"
DRY_RUN=0
...
usage() {
  cat <<'USAGE'
Build the v0 Pastey native package.

Usage:
  scripts/package-pastey.sh [--bundle <kind>] [--dry-run]
...
Environment:
  PASTEY_BUNDLE_KIND               Override the Tauri bundle kind.
  PASTEY_CREATE_UPDATER_ARTIFACTS  Set to 1 for signed Tauri updater artifacts.
...
if [[ -z "$BUNDLE_KIND" ]]; then
  case "$(uname -s)" in
    ...
    MINGW*|MSYS*|CYGWIN*|Windows_NT)
      ...
      ;;
    *)
      echo "Unable to infer bundle kind for host OS $(uname -s); set PASTEY_BUNDLE_KIND or --bundle" >&2
      exit 2
      ;;
  esac
fi
...
case "$BUNDLE_KIND" in
  appimage|dmg|nsis)
    ;;
  *)
    echo "Unsupported Pastey bundle kind: $BUNDLE_KIND" >&2
...
echo "==> Installing Pastey dependencies if needed"
if [[ ! -d "$ROOT_DIR/node_modules" ]]; then
  (cd "$ROOT_DIR" && npm ci)
fi

echo "==> Building Pastey web assets"
(cd "$ROOT_DIR" && npm run build)

if [[ "$CREATE_UPDATER_ARTIFACTS" == "1" ]]; then
  if [[ -z "${TAURI_SIGNING_PRIVATE_KEY:-}" && -z "${TAURI_SIGNING_PRIVATE_KEY_PATH:-}" ]]; then
    echo "PASTEY_CREATE_UPDATER_ARTIFACTS=1 requires TAURI_SIGNING_PRIVATE_KEY or TAURI_SIGNING_PRIVATE_KEY_PATH" >&2
    exit 1
  fi
