...
REPO="${PASTEY_REPO:-alexanderwanyoike/pastey}"
VERSION="${PASTEY_VERSION:-latest}"
ASSET_NAME="${PASTEY_ASSET_NAME:-pastey-x86_64.AppImage}"
INSTALL_DIR="${PASTEY_INSTALL_DIR:-$HOME/.local/bin}"
STATE_DIR="${PASTEY_STATE_DIR:-${XDG_STATE_HOME:-$HOME/.local/state}/pastey}"
BIN_NAME="${PASTEY_BIN_NAME:-pastey}"
CHECK_ONLY=0
DRY_RUN=0
FORCE=0
...
usage() {
  cat <<'USAGE'
Install or update the Pastey Linux AppImage from GitHub releases.

Usage:
  scripts/install-pastey.sh [--check] [--update] [--force] [--dry-run]
...
Environment:
  PASTEY_REPO         GitHub repository, default alexanderwanyoike/pastey.
  PASTEY_VERSION      Release tag to install, or latest. Default latest.
  PASTEY_INSTALL_DIR  Install directory. Default $HOME/.local/bin.
  PASTEY_STATE_DIR    State directory for recorded version.
  PASTEY_ASSET_NAME   Release asset. Default pastey-x86_64.AppImage.
  PASTEY_BIN_NAME     Command name. Default pastey.
...
if [[ -z "$RESOLVED_VERSION" || "$RESOLVED_VERSION" == "latest" ]]; then
  echo "Unable to resolve a Pastey release tag" >&2
  exit 1
fi
...
if [[ "$CHECK_ONLY" -eq 1 ]]; then
  if [[ "$CURRENT_VERSION" == "$RESOLVED_VERSION" && -x "$TARGET_BIN" ]]; then
    echo "Pastey is up to date."
  else
    echo "Pastey update available: ${CURRENT_VERSION:-not installed} -> $RESOLVED_VERSION"
  fi
  exit 0
fi
